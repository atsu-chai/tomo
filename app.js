...
function generateDormTemplate({ floors, roomsPerFloor, density }) {
  ...
  for (let f=1; f<=floors; f++) {
    ...
    for (let r=1; r<=roomsPerFloor; r++) {
      ...
      if (occupied) {
        const resident = {
          id: uid('res'), nickname: `住人${f}-${r}`, roomNumber, gradeOrRole: GRADES[Math.floor(Math.random()*GRADES.length)],
          hobbyTags: [HOBBIES[Math.floor(Math.random()*HOBBIES.length)]], lifestyleTags: [LIFESTYLES[Math.floor(Math.random()*LIFESTYLES.length)]],
          comment: '', avatarType: 'circle', avatarColor: `hsl(${Math.random()*360} 80% 75%)`, isSelf: false, relationshipStates: [], createdAt: now(), updatedAt: now()
        };
